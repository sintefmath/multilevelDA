...
output_path =  os.path.join(os.path.realpath(os.path.dirname(__file__)),"PracticalCost/"+timestamp )
...
for l in ls:
    doubleJetCase = DoubleJetCase.DoubleJetCase(gpu_ctx, DoubleJetCase.DoubleJetPerturbationType.SteadyState, ny=2**l, nx=2**l)
...
# np.savetxt(output_path+"/costs_partnered.txt", np.mean(costsPartnered, axis=-1))
