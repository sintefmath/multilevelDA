...
def imshow3(etahuhv, negative_vlim=True,  eta_vlim=1, huv_vlim=25, cmap="coolwarm", title=None):
    ...
    im = axs[0].imshow(etahuhv[0], vmin=-negative_vlim*eta_vlim, vmax=eta_vlim, cmap=cmap, origin="lower")
    plt.colorbar(im, ax=axs[0], shrink=0.5)
    axs[0].set_title("$\eta$", fontsize=15)

    im = axs[1].imshow(etahuhv[1], vmin=-negative_vlim*huv_vlim, vmax=huv_vlim, cmap=cmap, origin="lower")
    plt.colorbar(im, ax=axs[1], shrink=0.5)
    axs[1].set_title("$hu$", fontsize=15)

    im = axs[2].imshow(etahuhv[2], vmin=-negative_vlim*huv_vlim, vmax=huv_vlim, cmap=cmap, origin="lower")
...
def imshow3var(est_var, eta_vlim=1e-3, huv_vlim=5, title=None):
